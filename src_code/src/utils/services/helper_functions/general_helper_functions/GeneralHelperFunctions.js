...
async function SendVerificationCode(
  ...
  const imagePath = path.join(__dirname, "../../../../assets/ETCLogo.png");
  ...
  const transporter = nodemailer.createTransport({
    ...
    auth: {
      ...
      pass: process.env
        .EMAIL_APP_PW /** This is app password code for ecs gmail account when use nodemailer */,
    },
  });
...
async function GenerateDriveImagePublicUrl(drive, imageDriveFileId) {
  try {
    ...
    console.log(
      "ERROR: There is an error while generating drive image url: ",
      error
    );
    return null;
  }
...
async function DeleteDriveImage(drive, imageDriveFileId) {
  try {
    ...
    console.log(
      `ERROR: There is an error occur while deleting image with id ${imageDriveFileId}: `,
      error
    );
  ...
  try {
    ...
    const deletePromises = imageDriveFileIds.map(async (fileId) => {
      try {
        ...
        console.log(
          `ERROR: There is an error occur while deleting image with id ${fileId}: `,
          error
        );
        return `An error occurs while deleting image.`;
      }
    ...
    /** Log the error and return an error message */
    console.log(
      `ERROR: There is an error occur while deleting image with id ${imageDriveFileIds}: `,
      error
    );
...
function ConvertHEXStringToNumber(hexString, purpose = "tagId") {
    if(hexString && (typeof hexString !== 'string' || !/^[0-9A-Fa-f]+$/.test(hexString) || hexString.length !== 4)) {
      if(purpose === "tagId") {
