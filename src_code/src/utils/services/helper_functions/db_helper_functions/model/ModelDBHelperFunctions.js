...
async function GetModelInfoByModelId(db, modelId) {
  try {
    ...
    const type = await db
      .select("TYPE_NAME")
      .from("equipment_type")
      .where("PK_TYPE_ID", "=", model.FK_TYPE_ID)
      .first();
    ...
    console.log(
      "ERROR: There is an error retrieving model's information:",
      error
    );
...
async function GetEquipmentAvailableCount(
  db,
  modelId,
  typeId,
  startDate,
  ...
  try {
    const allReadyEquipmentsPromise = db("equipment")
    ...
    const reservedEquipmentsCountPromise = db("reserved_equipment")
      .leftJoin(
        "reservation",
        "reserved_equipment.FK_RESERVATION_ID",
        "=",
        "reservation.PK_RESERVATION_ID"
      )
      ...
      .first();
    const [allEquipments, reservedEquipmentsCount] = await Promise.all([
      allReadyEquipmentsPromise,
      reservedEquipmentsCountPromise,
    ]);
    ...
    console.log(
      "ERROR: There is an error while retrieve equipment's availability:",
      error
    );
