...
async function GetTypeInfoByName(db, equipment_type_name) {
  try {
    ...
    const item = await db
      ...
      .from("equipment_type")
      .where("TYPE_NAME", "=", equipment_type_name.trim())
      .first();
    ...
    console.log(
      "ERROR: There is an error while retrieving type information by name: ",
      error
    );
...
async function AddTypeToDatabase(db, equipment_type_name) {
  try {
    ...
    const [typeId] = await db("equipment_type").insert({
      TYPE_NAME: equipment_type_name,
    });

    /** Retrieve the inserted type information using the type ID */
    const addedType = await db("equipment_type")
      .select("PK_TYPE_ID", "TYPE_NAME")
      .where("PK_TYPE_ID", "=", typeId)
      .first();
    ...
    console.log(
      "ERROR: There is an error while adding type to the database: ",
      error
    );
    /** Return error message string */
...
async function GetTypeInfoByTypeId(db, typeId) {
  try {
    /** Retrieve type */
    const typePromise = await db("equipment_type")
      .select("PK_TYPE_ID AS typeId", "TYPE_NAME AS typeName")
      .where("PK_TYPE_ID", "=", typeId)
      .first();

    const modelsPromise = db("equipment_model")
      .select(
    ...
    const equipmentsPromise = db("equipment")
      .select(
    ...
    const [type, models, equipments] = await Promise.all([
      typePromise,
      modelsPromise,
      equipmentsPromise,
    ]);
    ...
    /** Log the error for debugging */
    console.log(
      "ERROR: There is an error while retrieving type by id: ",
      error
    );
    /** Return error message string */
    return "There is an error occur while retrieving type information.";
