...
async function MakeReservation(res, req) {
  ...
  try {
    ...
    const user = await Promise.resolve(
      dbHelpers.GetUserInfoBySchoolId(db, schoolId)
    );
    ...
    const reservedEquipmentInsertPromise = reservedEquipments.map(
      async (equipment) => {
    ...
    return responseBuilder.CreateSuccessful(
      res,
      null,
    ...
    return responseBuilder.ServerError(
      res,
      "There is an error while creating your reservation."
    );
  }
}
...
async function ReservationDateValidation(startDate, endDate, schoolId) {
  ...
  if (user.userRole === "Student") {
    ...
    try {
      ...
      const totalQuantityOfEquipmentStudentReserve = await db(
        "reserved_equipment"
      )
      ...
      console.log(
        "ERROR: There is an error while retrieve total quantity of equipment student reserve:",
        error
      );
...
async function UserValidation(schoolId) {
  try {
    ...
    /** Log error and return error */
    console.log(
      "ERROR: There is an error while validating user information for creating reservation:",
      error
    );
...
async function EquipmentValidation(equipment, startDate, endDate) {
  try {
    ...
    if (
      equipment.typeId === undefined ||
      equipment.modelId === undefined ||
    ...
    if (
      typeof equipment.typeId !== "number" ||
      typeof equipment.modelId !== "number" ||
      typeof equipment.quantity !== "number"
    ) {
    ...
    const availableCountPromise = dbHelpers.GetEquipmentAvailableCount(
      db,
      equipment.modelId,
      equipment.typeId,
      startDate,
    ...
    const [type, model, availableCount] = await Promise.all([
      typePromise,
      modelPromise,
    ...
    console.log(
      `ERROR: There is an error while validating equipment with the information '${equipment.name}' for creating reservation:`,
      error
    );
 ...
 * @returns {string|null} - Error message if validation fails, otherwise null.
 */
async function ReservedEquipmentsValidation(
  reservedEquipments,
  startDate,
  endDate,
  schoolId
  ...
  try {
    ...
    const reservedEquipmentValidatePromise = reservedEquipments.map(
      async (equipment) => {
        const equipmentError = await Promise.resolve(
          EquipmentValidation(equipment, startDate, endDate)
        );
        if (equipmentError) {
    ...
    /** Retrieve user information */
    const getUserInfoPromise = Promise.resolve(
      dbHelpers.GetUserInfoBySchoolId(db, schoolId)
    );
    ...
    if (user.userRole === "Student") {
      const totalEquipmentReservedObject = await db("reserved_equipment")
        .select(db.raw("SUM(QUANTITY) AS totalQuantity"))
        .whereIn("FK_RESERVATION_ID", function () {
          this.select("PK_RESERVATION_ID")
            .from("reservation")
    ...
    /** Log and return error */
    console.log(
      "ERROR: There is an error while validate reserved equipments for creating reservation:",
      error
    );
    return "There is an error while creating your reservation.";
  }
...
async function MakeReservationValidation(res, req) {
  try {
    ...
    const reservationDateError = await Promise.resolve(
      ReservationDateValidation(startDate, endDate, schoolId)
    );
    ...
    if (!Array.isArray(reservedEquipments)) {
      return responseBuilder.BadRequest(
        res,
        "Reserved equipment has a invalid type."
      );
    }

    /** Validate if equipment is valid (ensure equipment exists and requested quantity is not exceed the available quantity) */
    const reservedEquipmentError = await Promise.resolve(
      ReservedEquipmentsValidation(
        reservedEquipments,
        startDate,
        endDate,
        schoolId
    ...
    /** Log and return error */
    console.log(
      "ERROR: There is an error while validating your information to create a reservation:",
      error
    );
    return responseBuilder.ServerError(
      res,
