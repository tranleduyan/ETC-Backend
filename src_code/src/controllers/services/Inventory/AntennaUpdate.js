...
async function AntennaUpdate(res, req, antennaId) {
  try {
    ...
    const errors = await Promise.resolve(ValidateAntennaUpdate(res, req, antennaId));
    if(errors) {
      return errors;
    }
    ...
    /** Update antenna */
    await db("reader_location").where("PK_READER_TAG_ID", "=", antennaId.trim())
      .update({
    ...
    return responseBuilder.ServerError(res, "There is an error while updating antenna information.");
  }
}
...
async function ValidateAntennaUpdate(res, req, antennaId) {
  try {
    ...
    if(locationId) {
      const location = await Promise.resolve(dbHelpers.GetLocationInformationById(db, locationId));
    ...
    if(antennaId.trim() === newAntennaId.trim() && locationId === parseInt(antennaInformation.FK_LOCATION_ID, 10)) {
    ...
    /** If error, log error and return 503 */
    console.log("ERROR: There is an error while validating updating antenna information:", error);
    return responseBuilder.ServerError(res, "There is an error while updating antenna information.");
  }
}
