  ...
  if (name.length > 25) {
    if (type === "First") {
      return responseBuilder.BadRequest(
        res,
        "First name is longer than 25 characters."
      );
    }
    return responseBuilder.BadRequest(
      res,
      "Last name is longer than 25 characters."
    );
  }
...
async function ValidateEmail(res, emailAddress, userId) {
  ...
  if (!emailPattern.test(emailAddress.trim())) {
    return responseBuilder.BadRequest(
      res,
      "Please enter a valid spu email address."
    );
  }

  /** Retrieve user with the sign up email address to check for the uniqueness */
  const isUserWithRequestEmail = await Promise.resolve(
    dbHelper.GetUserInfoByEmailAddress(db, emailAddress.trim())
  );
...
async function ValidateSchoolId(res, schoolId, userId) {
  ...
  const isUserWithRequestSchoolId = await Promise.resolve(
    dbHelper.GetUserInfoBySchoolId(db, schoolId.trim())
  );

  if (
    isUserWithRequestSchoolId &&
    typeof isUserWithRequestSchoolId === "string"
  ) {
  ...
  if (
    isUserWithRequestSchoolId &&
    isUserWithRequestSchoolId.userId !== userId
  ) {
...
async function ValidateTagId(res, tagId, userId) {
  ...
  if (!availableTagId) {
    return responseBuilder.BadRequest(
      res,
      "There is no available tag id. Remove some."
    );
  }
  ...
  if (tagIdDec < 4096 || tagIdDec > 8191) {
    return responseBuilder.BadRequest(
      res,
      `Student tag ID must be from #1000 to #1FFF. Available Tag ID: ${availableTagId}`
    );
  }
  ...
  if (userWithNewTagId && userWithNewTagId.userId !== userId) {
    return responseBuilder.BadRequest(
      res,
      `Tag ID already exists. Available Tag ID: ${availableTagId}`
    );
  }
}
...
async function ValidateFaculty(res, req, user) {
  try {
    ...
    if (newUserRole.trim() !== "Faculty") {
      return responseBuilder.BadRequest(
        res,
        "Contact lab manager to help you."
      );
    }
    ...
    if (emailAddress.trim() !== user.emailAddress.trim()) {
      const emailAddressError = ValidateEmail(
        res,
        emailAddress.trim(),
        user.userId
      );
    ...
    return responseBuilder.ServerError(
      res,
      "There is an error while updating information."
    );
  }
}
...
async function ValidateAdmin(res, req, user) {
  ...
  const targetUser = await Promise.resolve(
    dbHelper.GetUserInfoBySchoolId(db, targetSchoolId.trim())
  );
  ...
  /** Validate email */
  if (emailAddress.trim() !== user.emailAddress.trim()) {
    const emailAddressError = await ValidateEmail(
      res,
      emailAddress.trim(),
      targetUser.userId
    );
  ...
  if (newSchoolId.trim() !== targetSchoolId) {
    const newSchoolIdError = await ValidateSchoolId(
      res,
      newSchoolId.trim(),
      targetUser.userId
    );
...
async function ValidateUpdateUserInformation(res, req, schoolId) {
  try {
    ...
    if (user.userRole === "Student") {
      return responseBuilder.BadRequest(
        res,
        "Contact lab administrator to help you."
      );
    ...
    /** If error, log error and return 503 */
    console.log(
      "ERROR: There is an error occur while updating user information:",
      error
    );
    return responseBuilder.ServerError(
      res,
...
async function UpdateUserInformation(res, req, schoolId) {
  try {
    ...
    const userBeforeChange = await Promise.resolve(
      dbHelper.GetUserInfoBySchoolId(db, schoolId.trim())
    );
    ...
    const user = await Promise.resolve(
      dbHelper.GetUserInfoByUserId(db, userBeforeChange.userId)
    );

    let responseObject = null;
    if (user.userRole !== "Admin") {
      responseObject = await Promise.resolve(
        dbHelper.GetUserInfoBySchoolId(db, newSchoolId.trim())
      );
    ...
    responseObject = await db("user_info")
      .select(
        ...
        db.raw(
          "CONCAT(COALESCE(FIRST_NAME, ''), ' ', COALESCE(LAST_NAME, ''), ' - ID: ', COALESCE(SCHOOL_ID, 'Not Found')) AS fullNameId"
        ),
    ...
    const responseInfo = responseObject.find(
      (user) => user.schoolId === newSchoolId.trim()
    );
    ...
    /** If error, log error and return 503 */
    console.log(
      "ERROR: There is an error while updating user information:",
      error
    );
    return responseBuilder.ServerError(
      res,
      "There is an error while updating user information."
    );
  }
}
